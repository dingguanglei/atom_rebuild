...
def weights_init(m):
    ...
    if classname.find('Conv') != -1:
        m.weight.data.xavier_normal(0.0, 0.01)
...
def define_G(input_nc, output_nc, ngf, norm='batch', use_dropout=False, gpu_ids=[], n_blocks=32):
    netG = None
...
def define_D(input_nc, ndf, norm='batch', use_sigmoid=False, gpu_ids=[], n_layers=4):
    netD = None
    use_gpu = len(gpu_ids) > 0
    norm_layer = get_norm_layer(norm_type=norm)
...
#         # 256 x 256
#         self.layer1 = Sequential(Conv2d(input_nc + output_nc, depth, kernel_size=4, stride=2, padding=1),
#                                  LeakyReLU(0.2))
#         # 128 x 128
...
#                                  LeakyReLU(0.2))
#         # 32 x 32
#         self.layer4 = Sequential(Conv2d(depth * 4, depth * 8, kernel_size=4, stride=1, padding=1),
#                                  BatchNorm2d(depth * 8),
#                                  LeakyReLU(0.2))
#         # 31 x 31
#         self.layer5 = Sequential(Conv2d(depth * 8, output_nc, kernel_size=4, stride=1, padding=1))
#         # 30 x 30
#
...
#     def __init__(self, input_nc=1, output_nc=1, depth=1, momentum=0.8):
...
#         self.depth = depth
#         self.d0 = Sequential(Conv2d(input_nc, depth, 3, 1, 1), LeakyReLU(0.2),
#                              Conv2d(depth, depth, 3, 1, 1), LeakyReLU(0.2))  # 256, 256, 16
#         self.d1 = Sequential(Conv2d(depth, depth * 2, 4, 2, 1), LeakyReLU(0.2),
#                              Conv2d(depth * 2, depth * 2, 3, 1, 1), LeakyReLU(0.2),
#                              BatchNorm2d(depth * 2, momentum=momentum))  # 128, 128, 32
#         self.d2 = Sequential(Conv2d(depth * 2, depth * 2, 4, 2, 1), LeakyReLU(0.2),
#                              BatchNorm2d(depth * 2, momentum=momentum),
#                              Conv2d(depth * 2, depth * 2, 3, 1, 1), LeakyReLU(0.2),
#                              BatchNorm2d(depth * 2, momentum=momentum))  # 64, 64, 32
#         self.d3 = Sequential(Conv2d(depth * 2, depth * 4, 4, 2, 1), LeakyReLU(0.2),
#                              BatchNorm2d(depth * 4, momentum=momentum),
#                              Conv2d(depth * 4, depth * 4, 3, 1, 1), LeakyReLU(0.2),
#                              BatchNorm2d(depth * 4, momentum=momentum))  # 32, 32, 64
#         self.d4 = Sequential(Conv2d(depth * 4, depth * 4, 4, 2, 1), LeakyReLU(0.2),
#                              BatchNorm2d(depth * 4, momentum=momentum),
#                              Conv2d(depth * 4, depth * 4, 3, 1, 1), LeakyReLU(0.2),
#                              BatchNorm2d(depth * 4, momentum=momentum))  # 16, 16, 64
#         self.d5 = Sequential(Conv2d(depth * 4, depth * 8, 4, 2, 1), LeakyReLU(0.2),
#                              BatchNorm2d(depth * 8, momentum=momentum),
#                              Conv2d(depth * 8, depth * 8, 3, 1, 1), LeakyReLU(0.2),
#                              BatchNorm2d(depth * 8, momentum=momentum))  # 8, 8, 128
#         self.d6 = Sequential(Conv2d(depth * 8, depth * 8, 4, 2, 1), LeakyReLU(0.2),
#                              BatchNorm2d(depth * 8, momentum=momentum),
#                              Conv2d(depth * 8, depth * 8, 3, 1, 1), LeakyReLU(0.2),
#                              BatchNorm2d(depth * 8, momentum=momentum))  # 4, 4, 128
#         self.d7 = Sequential(Conv2d(depth * 8, depth * 16, 4, 2, 1), LeakyReLU(0.2),
#                              BatchNorm2d(depth * 16, momentum=momentum),
#                              Conv2d(depth * 16, depth * 16, 3, 1, 1), LeakyReLU(0.2),
#                              BatchNorm2d(depth * 16, momentum=momentum))  # 2, 2, 256
#         self.d8 = Sequential(Conv2d(depth * 16, depth * 16, 4, 2, 1), LeakyReLU(0.2),
#                              BatchNorm2d(depth * 16, momentum=momentum),
#                              Conv2d(depth * 16, depth * 16, 3, 1, 1), LeakyReLU(0.2),
#                              BatchNorm2d(depth * 16, momentum=momentum),
#                             )  # 2, 2, 256
...
#                              Conv2d(depth * 16, depth * 16, 3, 1, 1), LeakyReLU(0.2),
#                              Dropout(0.1,True), BatchNorm2d(depth * 16, momentum=momentum))
#         self.u1 = Sequential(Upsample(scale_factor=2),
#                              Conv2d(depth * 8, depth * 8, 3, 1, 1), LeakyReLU(0.2),
#                              Dropout(0.1,True), BatchNorm2d(depth * 8, momentum=momentum))
#         self.u2 = Sequential(Upsample(scale_factor=2),
#                              Conv2d(depth * 4, depth * 4, 3, 1, 1), LeakyReLU(0.2),
#                              Dropout(0.1,True), BatchNorm2d(depth * 4, momentum=momentum))
#         self.u3 = Sequential(Upsample(scale_factor=2),
#                              Conv2d(depth * 4, depth * 4, 3, 1, 1), LeakyReLU(0.2),
#                              Dropout(0.1,True), BatchNorm2d(depth * 4, momentum=momentum))
#         self.u4 = Sequential(Upsample(scale_factor=2),
#                              Conv2d(depth * 2, depth * 2, 3, 1, 1), LeakyReLU(0.2),
#                              Dropout(0.1,True), BatchNorm2d(depth * 2, momentum=momentum))
...
#                              Dropout(0.1,True), BatchNorm2d(depth * 1, momentum=momentum))
#         # self.u7 = Sequential(Upsample(scale_factor=2),
#         #                      Conv2d(depth * 1, depth * 1, 3, 1, 1), LeakyReLU(0.2),
#         #                      Dropout(0.1), BatchNorm2d(depth * 1, momentum=momentum))
#         self.u7 = Sequential(Conv2d(depth * 1, output_nc, 5, 1, 2), Sigmoid())
