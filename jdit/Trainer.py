...
class GanTrainer(SupTrainer):
    ...
    def train_epoch(self):
        for iteration, batch in tqdm(enumerate(self.train_loader, 1)):
            ...
            input_cpu, ground_truth_cpu = batch[0], batch[1]
            ...
            if iteration == 1:
                self.watcher.images([self.input, self.fake, self.ground_truth], ["input", "fake", "real"],
                                    self.current_epoch,
                                    tag="Train",
                                    show_imgs_num=3,
                                    mode=self.mode)
    ...
    def valid(self):
        ...
        for iteration, batch in enumerate(self.cv_loader, 1):
            self.mv_inplace(batch[0], self.input)  # input data
            self.mv_inplace(batch[1], self.ground_truth)  # real data
        ...
        self.watcher.images([self.input, self.fake, self.ground_truth], ["input", "fake", "real"], self.current_epoch,
                            tag="Valid")
        self.netG.train()
        self.netD.train()
        self.netG.zero_grad()
