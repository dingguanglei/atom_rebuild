...
def train(training_data_loader, netG, netD, epoch, real_a, real_b, cv_data_loader=None):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        # Combined loss
        loss_d = - d_real.mean() + d_fake.mean() + gradient_penalty(netD, real_a, fake_b.detach())
        ...
        d_fake = netD(fake_ab)
        # loss_g_gan = criterionGAN(d_fake, True)
...
def gradient_penalty(netD, real_data, fake_data , LAMBDA=10):
    ...
    # disc_interpolates = netD(interpolates)
    disc_interpolates = netD(torch.cat((interpolates, real_data),1))
