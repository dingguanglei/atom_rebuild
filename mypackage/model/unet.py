...
class Unet_G(Module):
    def __init__(self, input_nc=1, output_nc=1, depth=32, norm_type="batch", active_type="ReLU"):
        super(Unet_G, self).__init__()
        self.norm = getNormLayer(norm_type)
        self.active = getActiveLayer(active_type)
        self.depth = depth
        self.d0 = Sequential(Conv2d(input_nc, depth, 7, 1, 3), self.active())  # 256, 256, 16
        self.d1 = Sequential(Conv2d(depth * 1, depth * 2, 5, 1, 2), self.active(), MaxPool2d(2, 2))  # 128, 128, 32
        self.d2 = Sequential(Conv2d(depth * 2, depth * 2, 3, 1, 1), self.active(), MaxPool2d(2, 2))  # 64, 64, 32
        self.d3 = Sequential(Conv2d(depth * 2, depth * 4, 3, 1, 1), self.active(), MaxPool2d(2, 2))  # 32, 32, 64
        self.d4 = Sequential(Conv2d(depth * 4, depth * 4, 3, 1, 1), self.active(), MaxPool2d(2, 2))  # 16, 16, 64
        self.d5 = Sequential(Conv2d(depth * 4, depth * 8, 3, 1, 1), self.active(), MaxPool2d(2, 2))  # 8, 8, 128
        self.d6 = Sequential(Conv2d(depth * 8, depth * 8, 3, 1, 1), self.active(), AvgPool2d(2, 2))  # 4, 4, 128
        self.d7 = Sequential(Conv2d(depth * 8, depth * 16, 3, 1, 1), self.active(), AvgPool2d(2, 2))  # 2, 2, 256
        self.d8 = Sequential(Conv2d(depth * 16, depth * 16, 3, 1, 1), self.active())  # 2, 2, 256

        self.u0 = Sequential(Upsample(scale_factor=2),
                             Conv2d(depth * 16, depth * 16, 3, 1, 1), self.active())
        self.u1 = Sequential(Upsample(scale_factor=2),
                             Conv2d(depth * 8, depth * 8, 3, 1, 1), self.active())
        self.u2 = Sequential(Upsample(scale_factor=2),
                             Conv2d(depth * 4, depth * 4, 3, 1, 1), self.active())
        self.u3 = Sequential(Upsample(scale_factor=2),
                             Conv2d(depth * 4, depth * 4, 3, 1, 1), self.active())
        self.u4 = Sequential(Upsample(scale_factor=2),
                             Conv2d(depth * 2, depth * 2, 3, 1, 1), self.active())
        self.u5 = Sequential(Upsample(scale_factor=2),
                             Conv2d(depth * 2, depth * 2, 3, 1, 1), self.active())
        self.u6 = Sequential(Upsample(scale_factor=2),
                             Conv2d(depth * 1, depth * 1, 5, 1, 2), self.active())
