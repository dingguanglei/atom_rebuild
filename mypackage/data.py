...
class AtomDataset(Dataset):
    def __init__(self, x_file_names, y_file_names, transform_list_input, x_dir_path, y_dir_path,transform_list_real = None):
        self.x = []
        self.y = []
...
# d = dset.ImageFolder(root=ROOT_PATH)


# trainLoader, testLoader, cvLoader = GetDataLoader(IMAGE_PATH, MASK_PATH_DIC["gaussian"], test_size=10,
#                                                       train_size=10,batch_size=2)
def CheckLoader(loader):
    c = 0
    for i, j in loader:
        a = transforms.Normalize([-1], [2])(i[0])
        a = transforms.ToPILImage()(a.reshape(1, 256, 256)).convert("L")
        b = transforms.Normalize([-1], [2])(j[0])
